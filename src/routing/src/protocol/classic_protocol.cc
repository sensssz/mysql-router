...
int ClassicProtocol::copy_packets(int sender, int receiver, fd_set *readfds,
  ...
  SocketOperationsBase *sender_op = from_server ? rdma_operations_ : socket_operations_;
  SocketOperationsBase *receiver_op = from_server ? socket_operations_ : rdma_operations_;
  if ((res = sender_op->read(sender, &buffer.front(), buffer_length)) <= 0) {
    if (res == -1) {
  ...
  if (!handshake_done) {
    ...
    if (buffer[4] == 0xff) {
      ...
      if (receiver_op->write_all(receiver, server_error.data(), server_error.size()) < 0) {
  ...
  if (receiver_op->write_all(receiver, &buffer[0], bytes_read) < 0) {
    log_debug("Write error: %s", get_message_error(errno).c_str());
    return -1;
  }
